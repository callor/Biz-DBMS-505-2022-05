...
-- 가장먼저 어떤 DB 를 사용할 것인지 알려줘야 한다
USE mydb;
DROP TABLE tbl_test;
-- Table 을 생성할때 칼럼의 이름은 사전에 있는 키워드는 가급작
-- 사용하지 않는 것이 좋다
-- 사전에 있는 키워드를 사용할때는 
...
대부분의 DBMS 들은 먼저 
Database 를 생성하고
사용자가 Database에 접근할 수 있는 권한을 부여하오
...
BIGINT : 8 Byte : -2의 7승 -1 부터 2의 7승까지
*/

/*
AUTO_INCREMENT 칼럼을 만들때 주의할 점
데이터 타입은 BIGINT 로 설정
가급적 PRIMARY KEY 로 설정할것
PK 아닌경우는 반드시 NOT NULL 과 UNIQUE 를 같이 설정한다
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','이몽룡','법학',2,'010','서울특별시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','이몽룡','컴공과',3,'010-111-111','익산시');
